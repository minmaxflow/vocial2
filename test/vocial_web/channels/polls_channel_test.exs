defmodule VocialWeb.PollChannelTest do 
  ...
  setup do 
    ...
    {:ok, poll} = Vocial.Votes.create_poll_with_options(%{title: "Sample Poll", user_id: user.id}, ["One", "Two", "Three"])

    {:ok, _, socket} = socket("user_id", %{user_id: user.id})
  ...
  test "vote replyes with status ok", %{socket: socket, poll: poll} do 
